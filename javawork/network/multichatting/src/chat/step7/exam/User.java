...
public class User extends Thread {
	...
	public void ioWork() {
		try {
			...
			for(int i=0; i<size; i++) {
				...
				sendMsg("old/"+user.nickname); //이미 접속한 사용자들한테 새로 접속한 사용자의 nickname을 보낸다.
			}
			userlist.add(this);
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void broadcast() {
		...
		for(int i=0; i<size; i++) {
			User user = userlist.get(i);
			user.sendMsg("new/"+nickname); //이미 접속한 사용자들한테 새로 접속한 사용자의 nickname을 보낸다.
		}
	}
	...
	public void run() {
		while (true) {
			try {
				...
				for(int i=0; i<size; i++) {
					User user = userlist.get(i);
					user.sendMsg("chat/"+nickname+" : "+resMsg+"\n"); //이미 접속한 사용자들한테 새로 접속한 사용자의 nickname을 보낸다.
					sendMsg("chat/"+user.nickname+" : "+resMsg+"\n");
				}
			} catch (Exception e) {
			}
		}
